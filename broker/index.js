...
aedes.authenticate = async function (client, id, secret, callback) {
  if (await sensorService.isAuth(id, secret.toString())) {
    callback(null, true);
    return 1;
  }
...
aedes.on("client", function (client) {
  ...
  sensorService.setStatus(client.id, "online");
});
aedes.on("clientDisconnect", function (client) {
  ...
  sensorService.setStatus(client?.id, "offline");
});
